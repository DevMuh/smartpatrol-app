...
<script>
    ...
    $('#1Kategori_izin').addClass('mm-active');
    $('#subm_Absensi').css({
        'box-shadow': 'none',
    ...
    $(document).ready(function() {
        var table = $('#tb_kategori_izin').DataTable({
            // order       : [[0, 'DESC']],
            dom: 'Bfrtip',
            buttons: [
                //     {
            ...
            processing: true,
            serverside: true,
            ajax: '<?= base_url('kategori_izin/ajax') ?>'
        });
    });
    ...
    $(document).on('submit', '#addKategori', function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url() ?>kategori_izin/add',
            method: 'POST',
            contentType: false,
            cache: false,
            processData: false,
            ...
            success: function(response) {
                ...
                if (type == 'success') {
                    $('#tb_kategori_izin').DataTable().ajax.reload(null, false)
    ...
    function edit(id) {
        $.ajax({
            url: '<?= base_url('kategori_izin/get_by_id?id=') ?>' + id,
            method: 'GET',
            success: function(response) {
    ...
    function update(id, e) {
        e.preventDefault()
        $.ajax({
            url: '<?= base_url('kategori_izin/update?id=') ?>' + id,
            method: 'POST',
