...
<script>
    ...
    $('#1Kategori_laporan').addClass('mm-active');
    $('#subm_Master').css({
        'box-shadow': 'none',
        'background': 'transparent',
        'color': 'white'
    });
    $('#2Kategori_laporan').css({
        'color': '#c81b1b',
        'border-radius': '4px',
        'background': 'white'
    });
    ...
    $(document).ready(function() {
        var table = $('#tb_kategori_laporan').DataTable({
            dom: 'Bfrtip',
            buttons: [
                //     {
                //     extend      : 'excelHtml5',
                //     className   : 'btn btn-primary',
                //     title       : 'Data Kategori Laporan',
                //     text        : 'Export Excel',
                //     exportOptions: {
                //         columns: [1,2,3,4],
                //         modifier: {
            ...
            processing: true,
            serverside: true,
            ajax: '<?= base_url('kategori_laporan/ajax') ?>'
        });
    });
    ...
    $(document).on('submit', '#addKategori', function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url() ?>kategori_laporan/add',
            method: 'POST',
            contentType: false,
            cache: false,
            processData: false,
            ...
            success: function(response) {
                ...
                if (type == 'success') {
                    $('#tb_kategori_laporan').DataTable().ajax.reload(null, false)
    ...
    function edit(id) {
        $.ajax({
            url: '<?= base_url('kategori_laporan/get_by_id?id=') ?>' + id,
            method: 'GET',
            success: function(response) {
    ...
    function update(id, e) {
        e.preventDefault()
        $.ajax({
            url: '<?= base_url('kategori_laporan/update?id=') ?>' + id,
            method: 'POST',
